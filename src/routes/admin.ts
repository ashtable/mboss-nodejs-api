...
/**
 * The admin's email from the session `mboss-web`
 * already enforced. It is an audit value, never a
 * credential — authentication is the bearer
 * token, and only `mboss-web` holds that.
 * Validating it as an address here means a
 * missing header fails loudly instead of writing
 * a placeholder into `Broadcast.createdBy`, which
 * the wire declares to be an email.
 *
 * The shared rule rather than a local one,
 * because it also normalizes: `Admin@Example.com`
 * and `admin@example.com` are one admin, and an
 * audit trail that stores them as two cannot be
 * grouped.
 */
...
/**
 * Fastify plugin for the admin console: the
 * waitlist list and stats, and broadcast
 * creation, sending and history.
 */
export function adminRoutes(deps: RouteDeps): FastifyPluginAsync {
  return async (scope) => {
    ...
    /**
     * Keyset-paginated subscriber list, with each
     * row's own sent-broadcast count.
     */
    app.route({
      method: 'GET',
      ...
      handler: async (request, reply) => {
        ...
        const after =
          cursor === undefined ? undefined : decodeSubscriberCursor(cursor);
        if (after === null)
          return badRequest(reply, 'cursor is not a cursor this API minted');

        const page = await deps.store.listSubscribers({
          status,
          q,
          after,
          limit,
        ...
        const sentCounts = await deps.store.countSentDeliveriesFor(
    ...
    /**
     * Subscriber counts by status, plus the total
     * across all of them.
     */
    app.route({
      method: 'GET',
      ...
      handler: async () => {
        ...
        const all = Object.values(counts).reduce(
    ...
    /**
     * Creates a broadcast, snapshotting its
     * audience, and enqueues the send in the same
     * request.
     */
    app.route({
      method: 'POST',
      ...
      handler: async (request, reply) => {
        ...
        if (audience.length === 0) {
          // A broadcast to nobody is never what
          // an admin meant, so it fails rather
          // than quietly creating an empty send.
          return badRequest(
            reply,
            'audience reaches no one a broadcast may be sent to',
          );
        }
        ...
        const broadcast = await deps.store.createBroadcastWithSnapshot({
          ...
          // The effective audience is stored, not
          // the requested one: the worker
          // re-checks each recipient's current
          // status against this column, and the
          // requested list would make that check
          // wrong.
          audience,
        ...
        // Enqueued only once the snapshot has
        // committed. A durable job pointing at a
        // broadcast whose transaction rolled back
        // would never find its rows.
    ...
    /**
     * Sends one email outside any broadcast, for
     * previewing content before a real send.
     */
    app.route({
      method: 'POST',
      ...
      schema: {
        ...
        response: { 200: TestSendResponseSchema },
      },
      handler: async (request) => {
        // No workflow id: a test send is
        // deliberately repeatable. An admin who
        // asks for a second one wants a second
        // email, so idempotency here would be a
    ...
    /**
     * All broadcasts, newest first, with their
    ...
    /**
     * One broadcast's full detail, including
     * delivery counts across every status.
     */
    app.route({
      method: 'GET',
