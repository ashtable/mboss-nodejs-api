...
describe('subscriber cursor', () => {
  const position = {
    createdAt: new Date('2026-08-16T12:34:56.789Z'),
    id: 'sub_1',
  };

  it('round-trips a keyset position', () => {
    expect(decodeSubscriberCursor(encodeSubscriberCursor(position))).toEqual(
      position,
    );
  });
  ...
  it.each([
    ...
    [
      'base64url of something with no separator',
      Buffer.from('nope').toString('base64url'),
    ],
    [
      'base64url of an unparseable date',
      Buffer.from('never|sub_1').toString('base64url'),
    ],
    [
      'base64url with an empty id',
      Buffer.from('2026-08-16T00:00:00.000Z|').toString('base64url'),
    ],
