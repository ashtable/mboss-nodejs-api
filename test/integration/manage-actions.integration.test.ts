...
/**
 * The manage actions are the one write
 * path whose whole effect at the
 * database level is the `pausedAt` /
 * `unsubscribedAt` columns, and those
 * columns never reach the wire — so no
 * response body and no in-memory double
 * can show what they hold. Only a real
 * row can.
 */
describe.skipIf(!databaseUrl)('manage actions (real Postgres)', () => {
  ...
  it('leaves exactly one timestamp set — the one naming the state the subscriber is now in', async () => {
    ...
    const row = () =>
      prisma.subscriber.findUniqueOrThrow({ where: { id: subscriber.id } });
    ...
    // Unsubscribing takes the pause timestamp
    // with it: these columns say where the
    // subscriber is now, not everywhere they
    // have been.
    expect((await act('unsubscribe')).json()).toEqual({
      status: 'unsubscribed',
    });
    ...
    // Three status writes and the manage link
    // still works: only a bounce revokes links.
