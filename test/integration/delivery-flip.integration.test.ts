...
describe.skipIf(!databaseUrl)('delivery flips and bounces (real Postgres)', () => {
  ...
  async function seedPendingDelivery(): Promise<{ broadcastId: string; subscriberId: string }> {
  ...
  it('leaves one terminal state when the same pending row is flipped concurrently', async () => {
    ...
    // Every caller sees the same answer, and it is whatever the one winning UPDATE wrote.
  ...
  it('changes nothing when the row has already left pending', async () => {
    ...
    const flip = (status: string, error?: string) =>
      app.inject({
        ...
        payload: { subscriberId, status, ...(error === undefined ? {} : { error }) },
      });

    expect((await flip('sent')).json()).toEqual({ status: 'sent' });
    expect((await flip('failed', 'too late')).json()).toEqual({ status: 'sent' });
  ...
  it('bumps tokenVersion exactly once across repeated bounce batches', async () => {
    ...
    const batch = () =>
      app.inject({
        ...
        headers: internalAuth,
        payload: [{ email: 'member@example.com', event: 'bounce', timestamp: 1_785_542_400 }],
      });
  ...
  it('returns the same subscriber to concurrent signups for one address', async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 5 }, () => store.findOrCreateSubscriber('racy@example.com')),
    );

    expect(new Set(results.map((result) => result.subscriber.id)).size).toBe(1);
